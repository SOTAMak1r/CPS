...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background):
    render_path = os.path.join(model_path, name, "ours_{}".format(iteration), "renders_r")
    gts_path = os.path.join(model_path, name, "ours_{}".format(iteration), "gt_test")
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, load_generate, sample_name):
    with torch.no_grad():
        ...
        print(gaussians._xyz.shape)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, load_from_generate=load_generate, generate_sample=sample_name)
        print(gaussians._xyz.shape)
        bg_color = [0, 0, 0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--load_generate", action="store_true")
    parser.add_argument("--sample_name", default="train_mask.pth", type=str)
    ...
    render_sets(model.extract(args), args.iteration, pipeline.extract(args), args.skip_train, args.skip_test, args.load_generate, args.sample_name)